...
/// Glowing digital clock. `large` scales it up for the landscape layout.
...
/// Alarm. Tap the bell to enable and set a time; when it fires it **pauses your music**
/// (rather than blaring a siren) and flashes. Tap the flashing bell — or "Dismiss" — to
/// clear it. `large` scales it up for the landscape layout.
struct AlarmView: View {
    ...
    var body: some View {
        TimelineView(.periodic(from: .now, by: 1)) { context in
            ...
            .buttonStyle(.plain)
            .onChange(of: now) { checkAlarm(now) }
            .onChange(of: enabled) { if !enabled { stopAlarm() } }
